...
class Trainer(object):
    ...
    def __init__(self, cfg):
        ...
        self.train_loader, self.val_loader, self.test_loader, _ = make_data_loader(cfg, **kwargs)
        ...
        # self.use_val_ce_loss = cfg.LOSS.USE_VAL_CE_LOSS
        # self.val_ce_loss = torch.nn.NLLLoss(reduction="mean")
    ...
    def training(self, epoch):
        ...
        print(f"\n=>Epochs {epoch}, learning rate = {self.scheduler.get_last_lr()[0]:0.3e}, previous best = {self.best_pred:0.4f}, EXP_NAME {self.cfg.EXPERIMENT.NAME}")

        train_loss = 0.0
        self.model.train()
        start_time = time.time()
        if hasattr(self.model, "training_step"):
            # ARPL_CS has its own training step method inside the model (kept for compatibility)
    ...
    def validation(self, epoch):
        ...
        self.saver.save_metrics(epoch+1, {'val_loss': test_loss})
...
def main(args, unknown):
    ...
    if cfg.EXPERIMENT.SKIP_EPOCHS:
        ...
        trainer.save_checkpoint(0, False)
    else:
